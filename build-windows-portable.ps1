...
<#
  Build a portable Windows distribution for Pandamonium.

  Output layout:
    dist\Pandamonium\Pandamonium.exe
    dist\Pandamonium\static\...
    dist\Pandamonium\scripts\...
    dist\Pandamonium\mcp_servers\...
    dist\Pandamonium\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name Pandamonium @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\Pandamonium" -ForegroundColor Green
