...
_HF_TOKEN_STATUS_SNIPPET = (
    ...
    'Add one in Pandamonium Cookbook -> Settings -> HuggingFace Token."; '
    'fi'
)
...
def setup_cookbook_routes() -> APIRouter:
        ...
        patterns = [
            ...
            (
                ...
                [
                    ...
                    {"label": "Pandamonium now rewrites MLX repo-id launches to a cached snapshot when one exists", "op": "manual"},
                ],
            ),
    ...
    async def _repair_cookbook_known_host(host: str, ssh_port: str | None = None) -> tuple[bool, str]:
        """Refresh Pandamonium' own known_hosts entry for a validated Cookbook host.
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        # When Pandamonium runs from a venv (e.g. native macOS install), put its bin
    ...
    async def model_cached(request: Request, host: str | None = None, model_dir: str | None = None, ssh_port: str | None = None, platform: str | None = None):
        ...
        async def _run_cached_scan_once():
            if host:
                ...
                # LOCAL scan: use sys.executable (the venv Python Pandamonium is already
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            # Put Pandamonium's own venv bin on PATH (local runs only) so the serve
            ...
            if not remote:
                ...
                if local_windows:
                    ...
                    # user PATH entries from the already-running Pandamonium process.
            ...
            if ("llama_cpp" in req.cmd or "llama-server" in req.cmd) and not local_windows_llama_cmd:
                ...
                runner_lines.append('# Auto-generated by Pandamonium Cookbook: a `llama-server` lookalike')
                ...
                if remote and _ollama_host in ("0.0.0.0", "::"):
                    runner_lines.append('echo "[odysseus] WARNING: remote Ollama will bind to ${ODYSSEUS_OLLAMA_HOST}:${ODYSSEUS_OLLAMA_PORT} so Pandamonium can reach it from this host."')
    ...
    def _cookbook_tasks_status_sync():
        ...
        for task in tasks:
            ...
            if task_platform == "windows" and remote:
                ...
                # as just "Locale: C / Ubuntu_Pandamonium ❯" and the agent
