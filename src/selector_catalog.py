...
def build_selector_catalog(
    ...
    selections.append({
        ...
        "kind": "agent",
        "target": "jarvis",
        "runtime": "Model-backed agent",
        "location": "Pandamonium",
        "capabilities": ["model"],
        "selectable": True,
        "reason": None,
    })
    ...
    for worker_id, details in worker_statuses.items():
        ...
        selections.append({
            ...
            "kind": kind,
            "target": worker_id,
            "runtime": next((name for capability, name in (
                ("codex", "Codex"), ("claude", "Claude"), ("hermes", "Hermes"),
                ("external_agent", "External agent"),
            ) if capability in capabilities), "Agent"),
            "location": _bounded_text(details.get("machine"), "Not reported", 80),
