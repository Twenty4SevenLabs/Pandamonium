  ...
  try {
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (pkg.installed && pkg.pip_update_available === false && pkg.name !== 'llama_cpp') {
        const tip = esc(pkg.update_note || pkg.status_note || `Found externally; update outside ${getBrandName()}.`);
    ...
    list.innerHTML = [
      _viewingRemote ? '' : _section(`${esc(getBrandName())} app`, `Run inside the ${esc(getBrandName())} app itself.`, _appDeps),
  ...
  if (dlBtn && dlInput) {
    ...
    const triggerDownload = async () => {
      ...
      if (looksGgufRepo && !pickerInclude) {
        ...
        if (!pickerInclude) {
          uiModule.showToast(`Pick a GGUF quant first. ${getBrandName()} will not download the whole GGUF repo without an include pattern.`);
          return;
        }
...
function _renderRecipes() {
  ...
  html += '</select>';
  html += '</div>';
  html += `<p class="memory-desc doclib-desc">Optional packages that extend ${esc(getBrandName())} capabilities.</p>`;
  ...
  html += '</div>';
  html += `<p class="memory-desc doclib-desc">Configure SSH servers, install ${esc(getBrandName())} keys, choose model directories, and set the default server. Local is this machine.</p>`;
