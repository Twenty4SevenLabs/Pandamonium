...
import {
  connectPortal as connectPortalFlow,
  portalUrlIssue,
...
async function submitPortal(event) {
  ...
  const urlIssue = portalUrlIssue(portalUrl);
  if (urlIssue) {
    urlInput?.focus();
    setProgress({
      state: 'error',
  ...
  const keyIssue = portalMasterKeyIssue(key);
  if (keyIssue) {
    input?.focus();
    setProgress({
      state: 'error',
  ...
  try {
    const result = await connectPortalFlow({ portalUrl, masterKey: key });
    key = '';
    if (!result.ok) {
      ...
      setProgress({
        ...
        detail: portalConnectFailureMessage(result),
      });
      return;
    }
...
function init() {
  ...
  byId('mad-mcp-connect-form')?.addEventListener('submit', submitPortal);
